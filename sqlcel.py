...
def add_df_src():
    ...
    Example:
                ...
                0
                tbl
    0 can be changed to an actual sheet name (must be in 1st position in workbook)
...
def exec_sql(sql):
    ...
    if list_of_cols[0] == "*":
        ...
        for x in range(0, len(sql_tbl)):
            ...
            dataframe.to_sql(sql_tbl[x], engine, if_exists='replace', index=0)
            # Every thing is now ready to run the SQL against the tables
        try:
            results = engine.execute(sql_code)
            final = pd.DataFrame(results, columns=list_of_cols)
        except Exception as e:
...
#  Main GUI Definition follows  #
#################################
# root = Tk()
# Requires ttkthemes module
# Linux: 'default', 'clam', 'scidsand', 'classic', 'scidblue''scidmint', 'scidgreen'
# Windows: 'xpnative', 'winnative', 'winxpblue'
# https://wiki.tcl-lang.org/page/List+of+ttk+Themes
root = ThemedTk(theme="scidsand")  # xpnative for Windows
...
btn_save = Button(frm_sql, text='Save', command=save_sql)
